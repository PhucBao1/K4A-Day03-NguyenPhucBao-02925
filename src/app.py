...
def run_react_agent(user_query: str, provider, mcp_server: MCPTravelServer) -> list:
    """
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                print(f"⚠️ [CHÚ Ý]: MCP Server trả về kết quả rỗng cho Tool '{tool_name}'.")
                ...
                if obs_data.get("status") == "SUCCESS":
                    if "message" in obs_data:
                        ...
                        final_answer = (
                            f"Kết quả tra cứu chuyến đi tới {obs_data.get('destination', '')} "
                            f"ngày {obs_data.get('date', '')}: {json.dumps(obs_data['data'], ensure_ascii=False)}"
                        )
                    else:
                        final_answer = f"Đã hoàn tất xử lý qua MCP Server: {json.dumps(obs_data, ensure_ascii=False)}"
                elif obs_data.get("status") == "NOT_FOUND":
                    final_answer = obs_data.get("message", "Không tìm thấy thông tin chuyến đi yêu cầu.")
                else:
...
if __name__ == "__main__":
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Câu hỏi chung: 'Chính sách hủy vé/hoàn tiền của bạn như thế nào?'")
        print("   - Tra cứu chuyến đi: 'Hãy tra cứu các chuyến bay đi Đà Nẵng vào ngày 20/09/2026'")
        print("   - Đặt vé: 'Đặt vé máy bay chuyến VN209 đi Đà Nẵng ngày 20/09/2026 cho Nguyễn Phúc Bảo'")
        ...
        print(f"--- 🏁 DEMO CHẠY THỬ 1 TEST CASE MẪU (TC02: Tra cứu chuyến bay) ---")
